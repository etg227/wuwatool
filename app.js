...
if(lr.length)renderBars($('lineMarginals'),lr,true);
else{
  $('lineMarginals').innerHTML='<div class="micro" style="margin-bottom:8px">尚未填写候选副词条 · 先展示各词条在当前面板下的理论边际收益（平均档）：</div><div class="bars" id="lineMarginalsFallback"></div>';
  renderBars($('lineMarginalsFallback'),std.map(x=>({label:`${LABEL[x.type]} ${x.value}${x.type.startsWith('flat')?'':'%'}`,gain:x.gain})),true);
}M.echoes.forEach((e,i)=>{const score=scoreEcho(ctx,e);$('miniScore'+i).textContent=score.toFixed(1);$('miniGrade'+i).textContent=grade(score)})}
