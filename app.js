...
function scoreEcho(ctx,e,echoes,index){const lines=e.sub.map((line,originalIndex)=>({line,originalIndex})).filter(x=>x.line.type&&x.line.value);if(!lines.length)return 0;const ref=Math.max(.000001,(standards(ctx)[0]?.gain||0)/100);let eq=0;for(const {originalIndex} of lines){const reduced=clone(echoes);reduced[index].sub[originalIndex]={type:'',value:0};const g=Math.max(-.999999,factorSet(ctx,echoes)/factorSet(ctx,reduced)-1);eq+=Math.log1p(g)/Math.log1p(ref)}return eq/5*100}
...
function calculate(){pullEchoes();const ctx=context();if(!(ctx.panel.atk||ctx.panel.hp||ctx.panel.def)){resetResults();return}const slot=+$('replaceSlot').value||0,next=clone(M.echoes);next[slot]=clone(M.candidate);const gain=(factorSet(ctx,next)/factorSet(ctx,M.echoes)-1)*100;$('overallGain').textContent=pct(gain);const cs=scoreEcho(ctx,M.candidate,next,slot);$('candidateScore').textContent=cs.toFixed(1);const cg=$('candidateGrade');if(cg)cg.textContent=`评级 ${grade(cs)}`;const s=model(ctx,aggregate(M.echoes)),cr=clamp(s.critRate/100,0,1),cd=Math.max(1,s.critDmg/100);$('critFactor').textContent=(1+cr*(cd-1)).toFixed(3);$('critState').textContent=`战斗态 ${s.critRate.toFixed(1)} / ${s.critDmg.toFixed(1)}`;const std=standards(ctx);if(!hasAnyCurrentSubstat()){$('bestStandard').textContent='等待录入词条';$('bestStandardNote').textContent='录入当前5只声骸的副词条后生成可实现建议';$('standardBars').innerHTML='<div class="micro">尚未录入声骸副词条。</div>'}else{const act=actionable(ctx,std);if(act.length){const t=act[0];$('bestStandard').textContent=LABEL[t.type];$('bestStandardNote').textContent=`当前可实现边际提升最高 · 声骸 ${t.slot+1} · ${t.mode==='upgrade'?'仍有提档空间':'可通过替换弱词条改善'}`;renderBars($('standardBars'),act.map(x=>({label:LABEL[x.type],gain:x.gain,tag:x.mode==='upgrade'?'可提档':'可替换'})),false)}else{$('bestStandard').textContent='暂无可实现升级';$('bestStandardNote').textContent='当前已录入词条没有可继续提升的合法空间';$('standardBars').innerHTML='<div class="micro">当前词条已饱和或没有正收益替换项。</div>'}}const lr=[];M.candidate.sub.forEach((l,j)=>{if(!l.type||!l.value)return;const r=clone(next);r[slot].sub[j]={type:'',value:0};lr.push({label:LABEL[l.type],gain:(factorSet(ctx,next)/factorSet(ctx,r)-1)*100})});renderBars($('lineMarginals'),lr,true);M.echoes.forEach((e,i)=>{const score=scoreEcho(ctx,e,M.echoes,i);$('miniScore'+i).textContent=score.toFixed(1);$('miniGrade'+i).textContent=grade(score)})}
